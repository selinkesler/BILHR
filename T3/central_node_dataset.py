    ...
    def image_cb(self,data):
        ...
        if len(areas_list) != 0:
            ...
            try:
                ...
                cx_norm = np.around(cx/320, 1)
                cy_norm = np.around(cy/240, 1)
                self.BlobX = cx_norm
                self.BlobY = cy_norm
    ...
    def central_execute(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.key == 1 and self.state == 1 : # do we need data.state so that it wouln't save it
                ...
                LS_Pitch = self.joint_angles[XX]
                LS_Roll = self.joint_angles[XX]               

                # check if gor the right value
                print(LS_Roll)

                LS_Pitch_norm = np.around((LS_Pitch + 2.0857) / (2*2.0857), 5)
                LS_Roll_norm = np.around((LS_Roll + 0.3142) / (1.3265 + 0.3142), 5)

                ex = [LS_Pitch_norm, LS_Roll_norm, self.BlobX, self.BlobY]

                # to save in csv file 
                with open('train.csv','a') as fd:
                    np.savetxt(fd, ex,delimiter=',',fmt='%d')   
